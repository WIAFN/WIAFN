...
public class RangedEnemyAnimation : MonoBehaviour
{
    ...
    private bool _lastBodyOverridden;
    ...
    void LateUpdate()
    {
        ...
        if (_lastBodyOverridden != lookAtSpecified)
        {
            if (lookAtSpecified)
            {
                _animator.SetBool(WIAFNAnimatorParams.InFight, true);
                EnableAimConstraints();
            }
            else
            {
            ...
            _lastBodyOverridden = lookAtSpecified;
        }

        if (_lastBodyOverridden)
        {
            UpdateAimConstraints(_npc.LookAtOrder);
    ...
    private void EnableAimConstraints()
    {
        rig.weight = 1f;
        //headConstraint.weight = 1f;
        //bodyConstraint.weight = 0.7f;
    }
    ...
    private void UpdateAimConstraints(LookAtOrder lookAtOrder)
    {
        ...
        float angleToLookAt = Vector3.Angle(_npc.transform.forward, _npc.LookAtOrder.Position - _npc.transform.position);
        float targetDistance = 1.6f;
        if (angleToLookAt > 120f)
        {
            targetDistance *= 3f;
        }
        _rightArmIKTarget.position = rightArmClavicle.position + (lookAtOrder.PositionRaw - rightArmClavicle.position).normalized * targetDistance;
    }
